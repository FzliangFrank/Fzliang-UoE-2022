...
```{r}
# use loop to creat an arguement
L = ""
for( i in findThoughts(stm_20, topic = c(1), texts =  df$ProjectId, n = 5)[[2]]){
...
cooked_csv %>%
  filter(ProjectId == '8E93BE78-0E07-4E83-8C63-CCB21B4BB70A' | ProjectId == 'EC57FD55-DD19-4DC2-BC9C-2E6320D987B6' | ProjectId == 'E04E24C5-AE50-4B57-AACE-EB15717F5968' | ProjectId == 'D3C287D8-B051-4DFE-86BD-484968AABBD2' | ProjectId == 'F1D1AE3A-334D-4B03-B9F1-A754CF5776E7') %>%
...
```{r}
L = ""
for( i in findThoughts(stm_20, topic = c(3), texts =  complete$ProjectId, n = 20)[[2]]){
  L = paste0(L, "ProjectId == ", shQuote(i), collapse=" | ")
}
...
df %>%
  ...
  geom_point(aes(x = StartDate), color = "#363636") + 
  geom_point(aes(x = EndDate), color = "blue") + 
  labs(title = "5 Research Porject that were under the same Name") + 
...
```{r}
findThoughts(stm_20, topic = c(3), texts =  complete$abstractText, n = 5)
findThoughts(stm_20, topic = c(3), texts =  complete$Title, n = 5)
```
