...
stm_20 %>% tidy(matrix = "theta") %>%
  ...
  arrange(document) %>%
  left_join(meta) %>%
  filter(max_proportion > 0.5) %>% # purity filter parameter is any projects propotion that is 0.75
  ...
  dplyr::summarise(x = sum(AwardPounds)) %>% # this is anohter x
...
data %>%
  arrange(-x) %>%
  filter(x > quantile(x, 0.75)) %>%
  


# graph
palette_theme = "Purples"
...
stm_20 %>% tidy(matrix = "theta") %>%
  ...
  arrange(document) %>%
  left_join(meta) %>%
  filter(max_proportion > 0.75) %>% # purity filter parameter is any projects propotion that is 0.75
  dplyr::rename(topic = variable) %>%
  filter(topic == 6) %>% # this is the part filter topic 6 only
  group_by(LeadROName) %>%
  filter(LeadROName != "The Alan Turing Institute") %>%
  #dplyr::summarise(x = sum(AwardPounds), n = length(LeadROName)) %>%
  #arrange(-x) %>%
