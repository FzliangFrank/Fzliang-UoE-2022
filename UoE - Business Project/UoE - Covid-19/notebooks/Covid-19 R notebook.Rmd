...
with(df, 
     plot(x = date, y = cumCasesByPublishDate, type = "l")
     )
with(df, 
     plot(x = date, y = cumCasesBySpecimenDate, type = "l")
     )
lines(x = df$date, y = df$cumDeaths28DaysByDeathDate, col = "red")
with(df,
...
## slice data:
```{r}
filter <- (df$date >= "2020-02-01")&(df$date <= "2020-07-1")
block1 <- df[filter,]
block1
# pilar two cases werre included from 2nd of July
with(block1, plot(x = date, y = cumCasesBySpecimenDate))

f2 <- (df$date >= "2020-07-14")&(df$date <= "2020-11-15")
block2 <- df[f2,]
block2
with(block2, plot(x = date, y = cumCasesBySpecimenDate))
...
Correct: differences between cum cases by publish date or speciman date. You won't have this problem if you switch the matrix to by speciman date. 
...
```
```{R}
model.2s <- list(model.2_l4, model.2_ll3, model.2_ll4, model.2_W1, model.2_W1.3, model.2_W2, model.2_W2.3)
model.2s_str <- list("model_l4","model_ll3","model_ll4","model_W1", "model_W1.3", "model_W2", "model_W2.3")
aic <- unlist( lapply(model.2s, AIC))
list2 <- cbind(model.2s_str, aic)
list2
```
